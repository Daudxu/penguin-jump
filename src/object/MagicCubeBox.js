...
import box_middle from '../assets/images/box_middle.png';
import box_bottom from '../assets/images/box_bottom.png';
import TWEEN from '@tweenjs/tween.js';
import {animateFrame} from "../utils/TweenUtil";
...
export default class MagicCubeBox extends Box {
  ...
  initBox() {
    ...
    const topMaterial = new THREE.MeshBasicMaterial({
      map: new THREE.TextureLoader().load(box_top),
    });
    const middleMaterial = new THREE.MeshBasicMaterial({
      map: new THREE.TextureLoader().load(box_middle),
    });
    const bottomMaterial = new THREE.MeshBasicMaterial({
      map: new THREE.TextureLoader().load(box_bottom),
    });

    const topGeometry = new THREE.BoxGeometry(this.height * 1.5, height, this.height * 1.5);
    const middleGeometry = new THREE.BoxGeometry(this.height * 1.5, height, this.height * 1.5);
    const bottomGeometry = new THREE.BoxGeometry(this.height * 1.5, height, this.height * 1.5);
    ...
    this.mesh = new THREE.Mesh(totalGeometry, totalMaterial);
